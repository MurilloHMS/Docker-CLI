...
namespace ContainersArtsystem
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            string diretorio = settings.Diretorio;
            ConfiguraDiretorio(settings, diretorio);
            ...
            while (true)
            {
                ...
                Console.WriteLine("5. Configurar Diretório");
                ...
                switch (option)
                {
                    ...
                    case "4":
                        ...
                        break;
                    case "5":
                        ConfiguraDiretorio2(settings, diretorio);
                        break;
                    case "6":
                        Environment.Exit(0);
                        break;
        ...
        private static void ConfiguraDiretorio(Settings settings, string diretorio)
        {
            if (string.IsNullOrEmpty(diretorio))
            {
                Console.WriteLine("Não há diretórios configurados");
                Console.Write("Digite caminho do diretório de backup: ");
                string diretorioDigitado = Console.ReadLine();

                if (Path.Exists(diretorioDigitado))
                {
                    Console.WriteLine($"Diretorio informado {diretorioDigitado} é válido");
                    settings.Diretorio = diretorioDigitado;
                    Console.WriteLine("informações salvas com sucesso!");
                    Console.ReadKey();
                }
                else
                {
                    Console.WriteLine($"não foi possivel localizar o diretório informado {diretorioDigitado}");
                }
            }
        ...
        private static void ConfiguraDiretorio2(Settings settings, string diretorio)
        {
            Console.WriteLine($"Diretorio configurado {settings.Diretorio}");
            Console.Write("Digite caminho do diretório de backup: ");
            string diretorioDigitado = Console.ReadLine();
        ...
        static async Task CreateSqlServerContainerAsync(DockerClient dockerClient, string hostPort, string containerName, string sqlVersion)
        {
            ...
            string acceptEula = "Y";
            string colation = "Latin1_General_CP1_CI_AI";
            Settings settings = new Settings(); 
            // Pasta do executável
            ...
            string backupPath = Path.Combine(settings.Diretorio, "Backup_Clientes");

            // Verifica se a pasta Backup_clientes existe, senão cria
            ...
            // Verifica se a pasta com o nome do contêiner existe, senão cria
